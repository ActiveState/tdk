...
# Meta description A package providing access to the first block of string
# Meta description information resources in win32 executables.
# Meta as::notes   Might be a candidate for placement in Tcllib.
# Meta platform    tcl
# @@ Meta End
...
proc ::stringfileinfo::writeStringInfo {file array} {
	...
	#puts [format "\noff %x len %d end %x" $o $len [expr {$o+2*$len}]]
        ...
        if {($tlen + $tpad) > $len} {
	    #puts "  $x: too long"
	    return -code error "$x: Replacement is too large."
	    break
	}
	#puts "  $x: set to '$val($x)'"
